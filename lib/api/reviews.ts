...
function feedbackToReview(f: FeedbackItem, locationName: string): Review | null {
...
export async function getReviews(): Promise<Review[]> {
  ...
  const locRes = await nextServer.get<{ locations: Array<{ _id: string; name: string }> }>(
    "/locations",
    { headers, params: { page: 1, perPage: LOCATIONS_PAGE_SIZE } }
  );
